...
export const setUserInfo = (userInfo) => {
  if (userInfo) {
    ...
    Cookies.set(USER_INFO_KEY, JSON.stringify(userInfo), {expires: config.cookieExpires || 1})
    setStore(ACCESS_KEY, userInfo.access.join(','))
  } else {
    Cookies.set(USER_INFO_KEY, '', {expires: config.cookieExpires || 1})
    setStore(ACCESS_KEY, '')
  }
}
...
export const getUserInfoCache = () => {
  var userInfo = Cookies.get(USER_INFO_KEY)
...
export const getAccessCache = () => {
  var access = getStore(ACCESS_KEY)
  if (access) return access.split(',')
  else return []
}

export const parseAccess = (permissions) => {
  var access = []
